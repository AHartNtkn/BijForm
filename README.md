...
A useful source audit before making completion claims is:

```sh
rg -n "sorry|admit|axiom|unsafe|SyntaxToNat|ObjIso|ObjToSyntax|SyntaxToObj" BijForm/*.lean
```

That audit is a status check, not a no-`sorry` policy. Any future `sorry` must
mark an honest unfinished proof obligation, while `admit`, `axiom`, `unsafe`
proof substitutes, and hidden placeholder constructions are not valid
completion evidence.
