...
def normalExpCount : List NFSort → Nat
  | [] => 0
  | .appTerm :: Γ => normalExpCount Γ
  | .normalExp :: Γ => normalExpCount Γ + 1

def appTermVarToFin : {Γ : List NFSort} → Var Γ .appTerm → Fin (appTermCount Γ)
  | .appTerm :: _, Var.here => ⟨0, by simp [appTermCount]⟩
  | .appTerm :: _, Var.there v =>
      let i := appTermVarToFin v
      ⟨i.val + 1, by
        dsimp [appTermCount]
        omega⟩
  | .normalExp :: _, Var.there v =>
      let i := appTermVarToFin v
      ⟨i.val, by
        dsimp [appTermCount]
        exact i.isLt⟩

def appTermFinToVar : {Γ : List NFSort} → Fin (appTermCount Γ) → Var Γ .appTerm
  | [], i => False.elim (Nat.not_lt_zero i.val i.isLt)
  | .appTerm :: _, ⟨0, _⟩ => Var.here
  | .appTerm :: Γ, ⟨n + 1, h⟩ =>
      Var.there (appTermFinToVar (Γ := Γ) ⟨n, by
        dsimp [appTermCount] at h
        omega⟩)
  | .normalExp :: Γ, i =>
      Var.there (appTermFinToVar (Γ := Γ) ⟨i.val, by
        dsimp [appTermCount] at i
        exact i.isLt⟩)

theorem appTermFinToVar_toFin :
    ∀ {Γ : List NFSort} (i : Fin (appTermCount Γ)),
      appTermVarToFin (appTermFinToVar i) = i := by
  intro Γ
  induction Γ with
  | nil =>
      intro i
  ...
  | cons hd Γ ih =>
      cases hd <;> intro i
      · apply Fin.ext
        simp [appTermFinToVar, appTermVarToFin]
        exact congrArg Fin.val (ih ⟨i.val, by
          dsimp [appTermCount] at i
          exact i.isLt⟩)
      · cases i with
        | mk n h =>
...
theorem appTermVarToFin_toVar :
    ∀ {Γ : List NFSort} (v : Var Γ .appTerm),
      appTermFinToVar (appTermVarToFin v) = v
  | .appTerm :: _, Var.here => rfl
  | .appTerm :: _, Var.there v => by
      simp [appTermVarToFin, appTermFinToVar, appTermVarToFin_toVar v]
  | .normalExp :: _, Var.there v => by
      simp [appTermVarToFin, appTermFinToVar, appTermVarToFin_toVar v]

def normalExpVarToFin : {Γ : List NFSort} → Var Γ .normalExp → Fin (normalExpCount Γ)
  | .normalExp :: _, Var.here => ⟨0, by simp [normalExpCount]⟩
  | .appTerm :: _, Var.there v =>
      let i := normalExpVarToFin v
      ⟨i.val, by
        dsimp [normalExpCount]
        exact i.isLt⟩
  | .normalExp :: _, Var.there v =>
      let i := normalExpVarToFin v
      ⟨i.val + 1, by
...
def normalExpFinToVar : {Γ : List NFSort} → Fin (normalExpCount Γ) → Var Γ .normalExp
  | [], i => False.elim (Nat.not_lt_zero i.val i.isLt)
  | .appTerm :: Γ, i =>
      Var.there (normalExpFinToVar (Γ := Γ) ⟨i.val, by
        dsimp [normalExpCount] at i
        exact i.isLt⟩)
  | .normalExp :: _, ⟨0, _⟩ => Var.here
  | .normalExp :: Γ, ⟨n + 1, h⟩ =>
      Var.there (normalExpFinToVar (Γ := Γ) ⟨n, by
        dsimp [normalExpCount] at h
        omega⟩)

theorem normalExpFinToVar_toFin :
    ∀ {Γ : List NFSort} (i : Fin (normalExpCount Γ)),
      normalExpVarToFin (normalExpFinToVar i) = i := by
  intro Γ
  induction Γ with
  | nil =>
      intro i
  ...
  | cons hd Γ ih =>
      ...
      · cases i with
        | mk n h =>
          ...
          | succ n =>
              apply Fin.ext
              simp [normalExpFinToVar, normalExpVarToFin]
              exact congrArg Fin.val (ih ⟨n, by
                dsimp [normalExpCount] at h
                omega⟩)
      · apply Fin.ext
...
theorem normalExpVarToFin_toVar :
    ∀ {Γ : List NFSort} (v : Var Γ .normalExp),
      normalExpFinToVar (normalExpVarToFin v) = v
  | .normalExp :: _, Var.here => rfl
  | .appTerm :: _, Var.there v => by
      simp [normalExpVarToFin, normalExpFinToVar, normalExpVarToFin_toVar v]
  | .normalExp :: _, Var.there v => by
      simp [normalExpVarToFin, normalExpFinToVar, normalExpVarToFin_toVar v]
