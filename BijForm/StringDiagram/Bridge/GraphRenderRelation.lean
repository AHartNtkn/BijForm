...
theorem GraphRenderRelated.connectChild_nodeIncident
    ...
    ∀ (node :
      ...
      (endpointOrder G (st.connectChild hpending mate hmate)).get
        ...
        (G.raw.incident
          ((nodeOrder (st.connectChild hpending mate hmate)).get
            (Fin.cast hchildNodeLength node))).get
          (Fin.cast (hnodeIncidentFields.node_incident_length node) slot) := by
  let rendererMate := st.restLabelIndex hpending mate
  let ok := st.connect_compatible hpending mate hmate
  let horderTrace :=
    connectChild_orderTrace rst st hpending mate hmate hids
  ...
  let childGraphSlot :
      Fin (G.raw.incident
        ((nodeOrder (st.connectChild hpending mate hmate)).get
          childNode)).length :=
    Fin.cast (hnodeIncidentFields.node_incident_length node) slot
  let oldGraphSlot :
...
theorem GraphRenderRelated.connectChild_edgeLabel
  ...
  have hidx : horderTrace.edge.rightIndex edge = childEdge := by
    exact fin_eq_of_val_eq rfl
  simpa [childEdge, hidx] using AppendTraceRelation.get hlabelRel edge
      ...
      exact list_get_of_eq_of_val_eq
        ...
        (by
          have hleft :
              ((Diag.connectStep rendererMate ok rst).edges.get edge).left =
                activeId := by
            have hrenderLeft : horderTrace.renderEdge.left = activeId := by
              simp [horderTrace, connectChild_orderTrace]
            exact (congrArg RenderEdge.left hnewEdge).trans hrenderLeft
          simpa [childEndpoint, oldEndpoint] using hleft)
    calc
      G.raw.endpointEdge
          ((endpointOrder G (st.connectChild hpending mate hmate)).get
            childEndpoint) =
        ...
        (by
          have hright :
            ...
            have hrenderRight :
                horderTrace.renderEdge.right = restIds.get idx := by
              simp [horderTrace, connectChild_orderTrace, idx, rendererMate]
            exact (congrArg RenderEdge.right hnewEdge).trans hrenderRight
          simpa [childEndpoint, oldEndpoint] using hright)
    calc
      G.raw.endpointEdge
          ((endpointOrder G (st.connectChild hpending mate hmate)).get
            childEndpoint) =
...
theorem GraphRenderRelated.budChild_endpointLabel
  ...
  have hidx : horderTrace.endpoint.rightIndex endpoint = childEndpoint := by
    exact fin_eq_of_val_eq rfl
  simpa [childEndpoint, hidx] using AppendTraceRelation.get hendpointRel endpoint
...
theorem GraphRenderRelated.budChild_edgeRight
  ...
  · have hnewVal : edge.val = rst.edges.length := by
    ...
    have hrawGe :
      ...
      have hfreshGe := Diag.freshNodeEndpoints_mem_ge hfreshMem
      simpa [nodeEndpoints, renderNode, hnextEndpoint] using
        hfreshGe
    have hrightBound :
        ((Diag.budStep renderNode entry ok rst).edges.get edge).right <
          rst.endpoints.length + Sig.arity renderNode := by
      have hbound := hchildEdgeBounds.right edge
    ...
    have hendpoint :
      ...
      rw [hedgeSlotIdx] at hendpointRaw
      exact hendpointRaw
    calc
      G.raw.endpointEdge
          ((endpointOrder G
...
theorem GraphRenderRelated.budChild_nodeLabel
  ...
  let childNode :
      Fin (nodeOrder
        (st.budChild hpending node slot hmate hunseen)).length :=
    Fin.cast hchildNodeLength renderIdx
  have hidx : horderTrace.node.rightIndex renderIdx = childNode := by
    exact fin_eq_of_val_eq rfl
  simpa [childNode, hidx] using AppendTraceRelation.get hlabelRel renderIdx
