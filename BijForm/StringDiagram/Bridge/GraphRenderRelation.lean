...
theorem GraphRenderRelated.endpointLabel_from_appendTrace
    {G : OpenPortHypergraph Sig boundary}
    ...
    (hrel : GraphRenderRelated G rst st)
    {newRenderEndpoints : List Sig.Port}
    {newGraphEndpoints : List (Fin G.raw.endpointCount)}
    (trace : AppendTrace rst.endpoints childRst.endpoints newRenderEndpoints
      (endpointOrder G st) (endpointOrder G childSt) newGraphEndpoints)
    (hchildEndpointLength :
      childRst.endpoints.length = (endpointOrder G childSt).length)
    (hnew :
      ∀ (renderEndpoint : Fin newRenderEndpoints.length)
        (graphEndpoint : Fin newGraphEndpoints.length),
        renderEndpoint.val = graphEndpoint.val →
        newRenderEndpoints.get renderEndpoint =
          G.raw.endpointLabel (newGraphEndpoints.get graphEndpoint)) :
    ∀ endpoint : Fin childRst.endpoints.length,
      childRst.endpoints.get endpoint =
        G.raw.endpointLabel
          ((endpointOrder G childSt).get
            (listIndexCast (endpointOrder G childSt)
              hchildEndpointLength endpoint)) := by
  intro endpoint
  exact AppendTraceRelation.get_listIndexCast_of
    (trace := trace)
    (R := fun renderEndpoint graphEndpoint =>
      renderEndpoint = G.raw.endpointLabel graphEndpoint)
    (by
      intro oldEndpoint graphEndpoint hval
      have hidx : graphEndpoint = hrel.endpointIndex oldEndpoint := by
        exact fin_eq_of_val_eq hval.symm
      simpa [hidx] using hrel.endpoint_label oldEndpoint)
    (by
      intro renderEndpoint graphEndpoint hval
      exact hnew renderEndpoint graphEndpoint hval)
    hchildEndpointLength endpoint

theorem GraphRenderRelated.edgeLabel_from_appendTrace
    {G : OpenPortHypergraph Sig boundary}
    {frontier childFrontier : List Sig.Port}
    {rst : RenderState Sig frontier} {st : SearchState G frontier}
    {childRst : RenderState Sig childFrontier}
  ...
  exact AppendTraceRelation.get_listIndexCast_of
    (trace := trace)
    (R := fun renderEdge graphEdge =>
      renderEdge.label = G.raw.edgeLabel graphEdge)
    (by
...
theorem GraphRenderRelated.edgeEndpointBounds_of_appendTrace
    {G : OpenPortHypergraph Sig boundary}
    {frontier childFrontier : List Sig.Port}
    {rst : RenderState Sig frontier} {st : SearchState G frontier}
    {childRst : RenderState Sig childFrontier}
    ...
    {newRenderEndpoints : List Sig.Port}
    (endpointStep :
      AppendStep rst.endpoints childRst.endpoints newRenderEndpoints)
    {newRenderEdges : List (RenderEdge Sig)}
    {newGraphEdges : List (Fin G.raw.edgeCount)}
    (edgeTrace : AppendTrace rst.edges childRst.edges newRenderEdges
      (edgeOrder st) (edgeOrder childSt) newGraphEdges)
    (hnewLeft :
      ∀ (renderEdge : Fin newRenderEdges.length)
        (graphEdge : Fin newGraphEdges.length),
        renderEdge.val = graphEdge.val →
        (newRenderEdges.get renderEdge).left < childRst.endpoints.length)
    (hnewRight :
      ∀ (renderEdge : Fin newRenderEdges.length)
        (graphEdge : Fin newGraphEdges.length),
        renderEdge.val = graphEdge.val →
        (newRenderEdges.get renderEdge).right < childRst.endpoints.length) :
    childRst.EdgeEndpointBounds := by
  have hprefixEndpointLength :
      rst.endpoints.length ≤ childRst.endpoints.length := by
    have hlen := endpointStep.length
    omega
  have hleftBoundRel :
      AppendTraceRelation edgeTrace
        (fun renderEdge _ => renderEdge.left < childRst.endpoints.length) := by
    refine { prefix_rel := ?_, suffix_rel := ?_ }
    · intro oldEdge _ _
      exact Nat.lt_of_lt_of_le (hrel.edge_left_bound oldEdge)
        hprefixEndpointLength
    · intro renderEdge graphEdge hval
      exact hnewLeft renderEdge graphEdge hval
  have hrightBoundRel :
      AppendTraceRelation edgeTrace
        (fun renderEdge _ => renderEdge.right < childRst.endpoints.length) := by
    refine { prefix_rel := ?_, suffix_rel := ?_ }
    · intro oldEdge _ _
      exact Nat.lt_of_lt_of_le (hrel.edge_right_bound oldEdge)
        hprefixEndpointLength
    · intro renderEdge graphEdge hval
      exact hnewRight renderEdge graphEdge hval
...
theorem GraphRenderRelated.edgeEndpointSide_from_appendTrace
    {G : OpenPortHypergraph Sig boundary}
    {frontier childFrontier : List Sig.Port}
    {rst : RenderState Sig frontier} {st : SearchState G frontier}
    {childRst : RenderState Sig childFrontier}
    ...
    {newGraphEndpoints : List (Fin G.raw.endpointCount)}
    (endpointTrace :
      AppendTrace rst.endpoints childRst.endpoints newRenderEndpoints
        (endpointOrder G st) (endpointOrder G childSt) newGraphEndpoints)
    {newRenderEdges : List (RenderEdge Sig)}
    {newGraphEdges : List (Fin G.raw.edgeCount)}
    (edgeTrace : AppendTrace rst.edges childRst.edges newRenderEdges
      (edgeOrder st) (edgeOrder childSt) newGraphEdges)
    (side : RenderEdge Sig → Nat)
    (hprefixBound :
      ∀ edge : Fin rst.edges.length,
        side (rst.edges.get edge) < rst.endpoints.length)
    (hprefixSide :
      ∀ edge : Fin rst.edges.length,
        G.raw.endpointEdge
            ((endpointOrder G st).get
              (hrel.endpointIndex
                ⟨side (rst.edges.get edge), hprefixBound edge⟩)) =
          (edgeOrder st).get (hrel.edgeIndex edge))
    (hchildEndpointLength :
      childRst.endpoints.length = (endpointOrder G childSt).length)
    (hchildEdgeLength :
      childRst.edges.length = (edgeOrder childSt).length)
    (hchildSideBound :
      ∀ edge : Fin childRst.edges.length,
        side (childRst.edges.get edge) < childRst.endpoints.length)
    (hnew :
      ∀ (renderEdge : Fin newRenderEdges.length)
        (graphEdge : Fin newGraphEdges.length),
        renderEdge.val = graphEdge.val →
        ∀ hbound :
          side (newRenderEdges.get renderEdge) < childRst.endpoints.length,
          G.raw.endpointEdge
              ((endpointOrder G childSt).get
                (listIndexCast (endpointOrder G childSt)
                  hchildEndpointLength
                  ⟨side (newRenderEdges.get renderEdge), hbound⟩)) =
            newGraphEdges.get graphEdge) :
    ∀ edge : Fin childRst.edges.length,
      G.raw.endpointEdge
          ((endpointOrder G childSt).get
            (listIndexCast (endpointOrder G childSt) hchildEndpointLength
              ⟨side (childRst.edges.get edge), hchildSideBound edge⟩)) =
        (edgeOrder childSt).get
          (listIndexCast (edgeOrder childSt) hchildEdgeLength edge) := by
  intro edge
  exact
    (AppendTraceRelation.get_listIndexCast_of
      (trace := edgeTrace)
      (R := fun renderEdge graphEdge =>
        ∀ hbound : side renderEdge < childRst.endpoints.length,
          G.raw.endpointEdge
              ((endpointOrder G childSt).get
                (listIndexCast (endpointOrder G childSt)
                  hchildEndpointLength
                  ⟨side renderEdge, hbound⟩)) =
            graphEdge)
      (by
        intro prefixEdge graphEdge hval hbound
        have hidx : graphEdge = hrel.edgeIndex prefixEdge := by
          exact fin_eq_of_val_eq hval.symm
        let childEndpoint : Fin (endpointOrder G childSt).length :=
          listIndexCast (endpointOrder G childSt) hchildEndpointLength
            ⟨side (rst.edges.get prefixEdge), hbound⟩
        let prefixEndpoint : Fin (endpointOrder G st).length :=
          hrel.endpointIndex
            ⟨side (rst.edges.get prefixEdge), hprefixBound prefixEdge⟩
        have hendpoint :
            (endpointOrder G childSt).get childEndpoint =
              (endpointOrder G st).get prefixEndpoint := by
          exact endpointTrace.get_prefix_at_right_of_val_eq
            childEndpoint prefixEndpoint
            (by simp [childEndpoint, prefixEndpoint])
        calc
          G.raw.endpointEdge ((endpointOrder G childSt).get childEndpoint) =
          ...
          _ = (edgeOrder st).get (hrel.edgeIndex prefixEdge) :=
              hprefixSide prefixEdge
          _ = (edgeOrder st).get graphEdge := by
              rw [hidx])
      (by
        intro renderEdge graphEdge hval hbound
        exact hnew renderEdge graphEdge hval hbound)
      hchildEdgeLength edge)
      (hchildSideBound edge)

theorem GraphRenderRelated.nodeLabel_from_appendTrace
    {G : OpenPortHypergraph Sig boundary}
    {frontier childFrontier : List Sig.Port}
    {rst : RenderState Sig frontier} {st : SearchState G frontier}
    {childRst : RenderState Sig childFrontier}
  ...
  exact AppendTraceRelation.get_listIndexCast_of
    (trace := trace)
    (R := fun renderNode graphNode =>
      renderNode.label = G.raw.nodeLabel graphNode)
    (by
...
theorem GraphRenderRelated.nodeIncidentFields_from_appendTrace
    {G : OpenPortHypergraph Sig boundary}
    {frontier childFrontier : List Sig.Port}
    {rst : RenderState Sig frontier} {st : SearchState G frontier}
    {childRst : RenderState Sig childFrontier}
    ...
    {newRenderNodes : List (RenderNode Sig)}
    {newGraphNodes : List (Fin G.raw.nodeCount)}
    (nodeTrace : AppendTrace rst.nodes childRst.nodes newRenderNodes
      (nodeOrder st) (nodeOrder childSt) newGraphNodes)
    (hchildNodeLength :
      childRst.nodes.length = (nodeOrder childSt).length)
    (hchildIncidentBound :
      ∀ (node : Fin childRst.nodes.length)
        (slot : Fin ((childRst.nodes.get node).incident.length)),
        (childRst.nodes.get node).incident.get slot <
          childRst.endpoints.length)
    (hnewLength :
      ∀ (renderNode : Fin newRenderNodes.length)
        (graphNode : Fin newGraphNodes.length),
        renderNode.val = graphNode.val →
        (newRenderNodes.get renderNode).incident.length =
          (G.raw.incident (newGraphNodes.get graphNode)).length) :
    GraphRenderRelated.NodeIncidentFields G childRst childSt
      hchildNodeLength := by
  ...
  · intro renderIdx
    exact AppendTraceRelation.get_listIndexCast_of
      ...
      (by
        intro renderNode graphNode hval
        exact hnewLength renderNode graphNode hval)
      hchildNodeLength renderIdx
  · exact hchildIncidentBound

theorem GraphRenderRelated.nodeIncident_from_appendTrace
    {G : OpenPortHypergraph Sig boundary}
    {frontier childFrontier : List Sig.Port}
    {rst : RenderState Sig frontier} {st : SearchState G frontier}
    {childRst : RenderState Sig childFrontier}
    ...
    (hnewIncident :
      ∀ (renderNode : Fin newRenderNodes.length)
        ...
        renderNode.val = graphNode.val →
        ∀ (renderSlot :
            Fin ((newRenderNodes.get renderNode).incident.length))
          (hbound :
            (newRenderNodes.get renderNode).incident.get renderSlot <
              childRst.endpoints.length)
          (hlen :
            (newRenderNodes.get renderNode).incident.length =
              (G.raw.incident (newGraphNodes.get graphNode)).length),
          (endpointOrder G childSt).get
              (listIndexCast (endpointOrder G childSt)
                ...
                ⟨(newRenderNodes.get renderNode).incident.get renderSlot,
                  hbound⟩) =
            (G.raw.incident (newGraphNodes.get graphNode)).get
              (listIndexCast
                (G.raw.incident (newGraphNodes.get graphNode))
                hlen renderSlot)) :
    ∀ (renderIdx : Fin childRst.nodes.length)
      (renderSlot :
        Fin ((childRst.nodes.get renderIdx).incident.length)),
      (endpointOrder G childSt).get
          (listIndexCast (endpointOrder G childSt)
            hchildEndpointLength
            ⟨(childRst.nodes.get renderIdx).incident.get renderSlot,
              hnodeIncidentFields.node_incident_bound renderIdx renderSlot⟩) =
        (G.raw.incident
          ((nodeOrder childSt).get
            (listIndexCast (nodeOrder childSt) hchildNodeLength
              renderIdx))).get
          (listIndexCast
            (G.raw.incident
              ((nodeOrder childSt).get
  ...
  have hlen :
      (childRst.nodes.get renderIdx).incident.length =
        (G.raw.incident
          ((nodeOrder childSt).get
          (listIndexCast (nodeOrder childSt) hchildNodeLength
            renderIdx))).length :=
  ...
  exact
    (AppendTraceRelation.get_listIndexCast_of
      (trace := nodeTrace)
      (R := fun rendered graphNode =>
        ∀ (renderSlot : Fin rendered.incident.length)
          (hbound : rendered.incident.get renderSlot <
            childRst.endpoints.length)
          (hlen :
      ...
      (by
        intro renderNode graphNode hval renderSlot hbound hlen
        exact hnewIncident renderNode graphNode hval renderSlot hbound hlen)
      hchildNodeLength renderIdx)
      renderSlot
      (hnodeIncidentFields.node_incident_bound renderIdx renderSlot)
      hlen
...
theorem GraphRenderRelated.connectChild
  ...
  | cons activeId restIds =>
      ...
      let hchildEdgeBounds :=
        hrel.edgeEndpointBounds_of_appendTrace horderTrace.endpoint.left_step
          horderTrace.edge
          (by
            intro renderEdge graphEdge hval
            have hbound :
                  ...
                  simpa using
                    hrel.frontier_id_bound_of_mem
                      (by
                        ...
                        simp))
                hstepEndpointLength
            simpa [horderTrace, FirstPendingStep.orderDelta] using hbound)
          (by
            intro renderEdge graphEdge hval
            have hbound :
                restIds.get idx <
              ...
              exact Nat.lt_of_lt_of_eq
                ...
                hstepEndpointLength
            simpa [horderTrace, FirstPendingStep.orderDelta, idx, rendererMate]
              using hbound)
      have hchildNodeIncidentBound :
          ∀ (node : Fin (Diag.connectStep rendererMate ok rst).nodes.length)
            (slot :
              Fin (((Diag.connectStep rendererMate ok rst).nodes.get
      ...
      let hnodeIncidentFields :=
        hrel.nodeIncidentFields_from_appendTrace horderTrace.node
          hchildNodeLength hchildNodeIncidentBound
          (by
            intro renderNodeIdx _graphNode _hval
            exact fin_zero_elim renderNodeIdx)
      let hnodeIncident :=
        hrel.nodeIncident_from_appendTrace horderTrace.endpoint
          horderTrace.node hchildEndpointLength hchildNodeLength
          hnodeIncidentFields
          (by
            intro renderNodeIdx _graphNode _hval _renderSlot _hbound _hlen
            exact fin_zero_elim renderNodeIdx)
      refine
        { endpoint_nodup := ?_
          edge_nodup := ?_
      ...
      · exact hrel.endpointLabel_from_appendTrace horderTrace.endpoint
          hchildEndpointLength (by
            intro renderEndpoint _graphEndpoint _hval
            exact fin_zero_elim renderEndpoint)
      · have hactiveLabel := (st.pending_labels_cons hpending).1
        ...
        exact hrel.edgeLabel_from_appendTrace horderTrace.edge
          hchildEdgeLength (by
            intro renderEdge graphEdge _hval
            simpa [horderTrace, FirstPendingStep.orderDelta] using hlabelNew)
      · exact hchildEdgeBounds.left
      · exact hchildEdgeBounds.right
      · exact hrel.edgeEndpointSide_from_appendTrace horderTrace.endpoint
          horderTrace.edge (fun edge => edge.left) hrel.edge_left_bound
          hrel.edge_left hchildEndpointLength hchildEdgeLength
          hchildEdgeBounds.left (by
            intro renderEdge graphEdge hval hbound
            ...
            have hendpoint :
                (endpointOrder G (st.connectChild hpending mate hmate)).get
                    (listIndexCast
                      ...
                      hchildEndpointLength
                      ⟨activeId, by
                        simpa [horderTrace, FirstPendingStep.orderDelta] using
                          hbound⟩) =
                    ...
                    ((endpointOrder G
                      (st.connectChild hpending mate hmate)).get
                        (listIndexCast
                          ...
                          hchildEndpointLength
                          ⟨activeId, by
                            simpa [horderTrace, FirstPendingStep.orderDelta]
                              using hbound⟩)) =
      ...
      · exact hrel.edgeEndpointSide_from_appendTrace horderTrace.endpoint
          ...
          hchildEdgeBounds.right (by
            ...
            have hendpoint :
                (endpointOrder G (st.connectChild hpending mate hmate)).get
                    (listIndexCast
                      ...
                      hchildEndpointLength
                      ⟨restIds.get idx, by
                        simpa [horderTrace, FirstPendingStep.orderDelta, idx,
            ...
            simpa [horderTrace, FirstPendingStep.orderDelta, idx, rendererMate]
              using hcalc)
      · exact hrel.nodeLabel_from_appendTrace horderTrace.node hchildNodeLength
          (by
            intro renderNodeIdx _graphNode _hval
            exact fin_zero_elim renderNodeIdx)
      · intro node
        ...
        hrel.nodeIncidentFields_from_appendTrace horderTrace.node
          ...
          (by
            ...
            exact hchildValid.node_incident_bound
              ...
              (List.get_mem (Diag.budStep renderNode entry ok rst).nodes
                renderIdx)
              renderSlot)
          (by
            intro suffixNode graphNode hval
            simpa [horderTrace, FirstPendingStep.orderDelta, Diag.freshNodeEndpoints,
              renderNode] using (G.raw.incident_length node).symm)
      let hnodeIncident :=
        hrel.nodeIncident_from_appendTrace horderTrace.endpoint horderTrace.node
          ...
          (by
            ...
            have hnodeBound :
                nodeEndpoints.get freshSlot <
                  (Diag.budStep renderNode entry ok rst).endpoints.length := by
              simpa [freshSlot, horderTrace, FirstPendingStep.orderDelta,
            ...
            have hrawGe :
                rst.endpoints.length ≤ nodeEndpoints.get freshSlot := by
              have hfreshMem :
            ...
            simpa [horderTrace, FirstPendingStep.orderDelta, nodeEndpoints,
              ...
              calc
                G.raw.endpointEdge
                  ...
                  G.raw.endpointEdge ((G.raw.incident node).get slot) := by
                    exact congrArg G.raw.endpointEdge hendpoint
                _ = G.raw.endpointEdge active := hmate.2.symm
            simpa [horderTrace, FirstPendingStep.orderDelta, nodeEndpoints, entryIdx,
