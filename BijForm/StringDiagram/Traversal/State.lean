...
graph suffix lists; the append-trace shape itself is shared.
-/
structure FirstPendingStep.OrderDelta
    {G : OpenPortHypergraph Sig boundary}
    ...
    (newGraphNodes : List (Fin G.raw.nodeCount)) where
  endpoint :
  ...
  edge :
    AppendTrace rst.edges childRst.edges newRenderEdges
...
def FirstPendingStep.orderDelta
    {G : OpenPortHypergraph Sig boundary}
    {activeLabel : Sig.Port} {frontier : List Sig.Port}
    (rst : RenderState Sig (activeLabel :: frontier))
    (st : SearchState G (activeLabel :: frontier))
    ...
    match step with
    | FirstPendingStep.connect mate hmate =>
        FirstPendingStep.OrderDelta rst
  ...
  | connect mate hmate =>
      ...
      have hdelta := Diag.connectStep_delta rendererMate ok rst hids
      exact
        { endpoint :=
            { prefix_length := hendpointLength
              suffix_length := rfl
              left_step := by
                simpa [rendererMate, ok] using hdelta.endpoints
              right_step :=
                { eq_append := endpointOrder_firstPendingChild st hpending
                    (FirstPendingStep.connect mate hmate) } }
          edge :=
            { prefix_length := hedgeLength
              suffix_length := rfl
              left_step := by
                simpa [rendererMate, ok] using hdelta.edges
              right_step :=
                { eq_append := edgeOrder_firstPendingChild st hpending
                    (FirstPendingStep.connect mate hmate) } }
          node :=
            { prefix_length := hnodeLength
              suffix_length := rfl
              left_step := by
                simpa [rendererMate, ok] using hdelta.nodes
              right_step :=
                { eq_append := nodeOrder_firstPendingChild st hpending
                    (FirstPendingStep.connect mate hmate) } } }
  | bud node slot hmate hunseen =>
      ...
      have hdelta := Diag.budStep_delta renderNode entry ok rst hids
      exact
        { endpoint :=
            { prefix_length := hendpointLength
              suffix_length := by
              ...
              left_step := by
                simpa [renderNode, entry, ok] using hdelta.endpoints
              right_step :=
                { eq_append := endpointOrder_firstPendingChild st hpending
                    (FirstPendingStep.bud node slot hmate hunseen) } }
          edge :=
            { prefix_length := hedgeLength
              suffix_length := rfl
              left_step := by
                simpa [renderNode, entry, ok] using hdelta.edges
              right_step :=
                { eq_append := edgeOrder_firstPendingChild st hpending
                    (FirstPendingStep.bud node slot hmate hunseen) } }
          node :=
            { prefix_length := hnodeLength
              suffix_length := rfl
              left_step := by
                simpa [renderNode, entry, ok] using hdelta.nodes
              right_step :=
                { eq_append := nodeOrder_firstPendingChild st hpending
                    (FirstPendingStep.bud node slot hmate hunseen) } } }
...
the renderer facts needed by the branch implementations, while consumers can
ask only for preservation of `firstPendingChildState`.
-/
inductive RenderPrefixChildStep
    ...
    {rest : List (Fin ev.toOpenPortHypergraph.raw.endpointCount)}
    (hpending : sst.pending = active :: rest) :
    (step : FirstPendingStep ev.toOpenPortHypergraph sst.seenNode active rest) →
    RenderState Sig (sst.firstPendingChildFrontier hpending step) → Prop where
  | connect (mate : Fin rest.length)
      (hmate :
      ...
      (ok :
        Sig.compatible activeLabel
          (restLabels.get (sst.restLabelIndex hpending mate)))
      (hactiveEdge :
  ...
  | bud (node : Fin ev.toOpenPortHypergraph.raw.nodeCount)
      ...
      (hfrontier :
        restLabels ++
            Sig.nodePortsExcept
              (ev.toOpenPortHypergraph.raw.nodeLabel node)
              (budEntry node slot) =
      ...
      RenderPrefixChildStep hpending
        (FirstPendingStep.bud node slot hmate hunseen)
        (by
          simpa [firstPendingChildFrontier] using
...
theorem RenderPrefixRelated.firstPendingChild
    ...
    (hpending : sst.pending = active :: rest)
    {step : FirstPendingStep ev.toOpenPortHypergraph sst.seenNode active rest}
    {childRst : RenderState Sig (sst.firstPendingChildFrontier hpending step)}
    (hstep : RenderPrefixChildStep (rst := rst) hpending step childRst) :
    RenderPrefixRelated ev childRst
      (sst.firstPendingChildState hpending step) := by
  cases hstep with
  | connect mate hmate ok hactiveEdge hedgesLength hnodesLength =>
      have hchild :
          RenderPrefixRelated ev
            (Diag.connectStep (sst.restLabelIndex hpending mate) ok rst)
            (sst.connectChild hpending mate hmate) := by
        exact {
        pending_vals := by
          ...
          | cons activeId restIds =>
              ...
              have hrestLen : restIds.length = restLabels.length := by
        ...
        processed_prefix := by
          intro edge
          have hprefix :=
            fin_cons_prefix_iff hrel.processed_prefix hactiveEdge
              hedgesLength edge
      ...
      simpa [firstPendingChildState, firstPendingChildFrontier] using hchild
  | bud node slot hmate hunseen rendererNode entry ok hfrontier hincidentVals
      hslot hactiveEdge hnewNode hedgesLength hnodesLength =>
      have hunseenMem : node ∉ sst.seenNodes := by
        simpa [seenNode] using hunseen
      have hchild :
          RenderPrefixRelated ev
            (hfrontier.symm ▸ Diag.budStep rendererNode entry ok rst)
            (sst.budChild hpending node slot hmate hunseenMem) := by
        exact {
        pending_vals := by
          have hchildFrontierIds :
              (sst.budChild hpending node slot hmate hunseenMem).pending.map
                  (fun endpoint => endpoint.val) =
            ...
            | cons activeId restIds =>
                ...
                rw [hvals.2]
                have hmap :
                    (eraseFin (ev.toOpenPortHypergraph.raw.incident node)
                        slot).map (fun endpoint => endpoint.val) =
                      eraseFin
                        ((ev.toOpenPortHypergraph.raw.incident node).map
                          (fun endpoint => endpoint.val))
                        (listMapIndex (fun endpoint => endpoint.val)
                          (ev.toOpenPortHypergraph.raw.incident node)
                          slot) := by
                  simpa [listMapIndex] using
        ...
        seen_prefix := by
          intro candidate
          have hprefix :=
            fin_cons_prefix_iff hrel.seen_prefix hnewNode hnodesLength
              candidate
          simpa [budChild] using hprefix }
      simpa [firstPendingChildState, firstPendingChildFrontier, seenNode] using
        hchild
...
theorem IsoRelated.firstPendingChild
  ...
  | connect mate hmate rightMateEdge =>
      ...
      simpa [firstPendingChildState, firstPendingChildFrontier,
        firstPendingChildFrontier_eq] using hchild
  | bud node slot hmate hunseen rightMateEdge rightUnseen =>
      ...
      have hchild :
          IsoRelated e
            (left.budChild hpending node slot hmate leftUnseen)
            (hfrontier ▸
            ...
            rest.map e.endpointEquiv.toFun ++
              ...
              rest.map e.endpointEquiv.toFun ++
                eraseFin ((G.raw.incident node).map e.endpointEquiv.toFun)
                  (listMapIndex e.endpointEquiv.toFun (G.raw.incident node)
                    slot) := by
                congr 1
                have hincident :
                ...
                exact eraseFin_eq_of_eq_of_val_eq hincident
                  (PortHypergraphIso.incidenceSlotPreserved e node slot)
                  (listMapIndex e.endpointEquiv.toFun
                    (G.raw.incident node) slot)
                  (by
                    simp [PortHypergraphIso.incidenceSlotPreserved,
                      listMapIndex])
            _ = rest.map e.endpointEquiv.toFun ++
                (eraseFin (G.raw.incident node) slot).map
                  e.endpointEquiv.toFun := by
                simpa [listMapIndex] using
                  (map_eraseFin e.endpointEquiv.toFun
                    (G.raw.incident node) slot).symm
            _ =
                (rest ++ eraseFin (G.raw.incident node) slot).map
                  e.endpointEquiv.toFun := by
                rw [List.map_append]
