...
theorem unaryTagToEntry_entry (entry : Sig.Entry)
    (hentry : Sig.arity entry.1 = 1) :
    data.unaryTagToEntry (data.unaryIso.toFun ⟨entry, hentry⟩) = entry := by
  exact congrArg Subtype.val (data.unaryIso.left_inv ⟨entry, hentry⟩)
...
theorem nonUnaryTagToEntry_entry (entry : Sig.Entry)
    (hentry : Sig.arity entry.1 ≠ 1) :
    data.nonUnaryTagToEntry (data.nonUnaryIso.toFun ⟨entry, hentry⟩) = entry := by
  exact congrArg Subtype.val (data.nonUnaryIso.left_inv ⟨entry, hentry⟩)
