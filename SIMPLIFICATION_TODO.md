...
- [x] Factor graph-render relation into ordered trace machinery.
  ...
  - Partial: `GraphRenderRelated.connectChild_nodeIncident` now owns the
  ...
  - Partial: `GraphRenderRelated.connectChild_edgeLeft` now owns the connect
    ...
    transport locally.
  - Partial: `GraphRenderRelated.connectChild_edgeLeft` now consumes
    `AppendTraceRelation`, so the full-edge old/new split for left-endpoint
    incidence is owned by the generic append-relation proof path.
...
- [ ] Introduce declarative render deltas for connect and bud.
  ...
  - Partial: `Renderer.Steps.RenderDelta` now owns connect/bud endpoint, edge,
    ...
    projections. Connect's unchanged-node and owner invariant proofs now use
    public step facts instead of unfolding the step. The connect edge/frontier
    invariants and bud invariant families still need to be derived through the
    delta model before this item is complete.
